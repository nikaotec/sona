...
class OnboardingScreen extends StatefulWidget {
  ...
  const OnboardingScreen({
    super.key,
    this.isEditMode, // Se não for passado, assume que é onboarding inicial
  });
...
class _OnboardingScreenState extends State<OnboardingScreen>
  ...
  void _finishOnboarding() async {
    if (widget.isEditMode == null || widget.isEditMode == false) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          // Botão voltar
          if (_currentPage > 0 &&
              (widget.isEditMode == null || widget.isEditMode == false)) // Onboarding inicial, a partir da segunda tela
            AnimatedButton(
              onPressed: _previousPage,
              isIconButton: true,
              child: const Icon(Icons.arrow_back, color: Colors.white),
            ).animate().fadeIn().slideX(begin: -0.3, end: 0)
          else if (widget.isEditMode != null && widget.isEditMode! )// Modo de edição
            AnimatedButton(
              onPressed: () {
                if (_currentPage == 0) {
                  context.go(
                    "/profile",
                  ); // Se na primeira página, volta para o perfil
                } else {
                  _previousPage(); // Volta para a página anterior do onboarding
                }
              },
              isIconButton: true,
              child: const Icon(Icons.arrow_back, color: Colors.white),
            ...
            const SizedBox(
              width: 48,
            ), // Espaço para alinhar quando não há botão voltar
          ...
          if (widget.isEditMode != null && widget.isEditMode!)
            AnimatedButton(
              onPressed:
                  () => context.go(
                    "/categories",
                  ), // Sempre vai para categorias no modo edição
              isIconButton: true,
            ...
            const SizedBox(
              width: 48,
            ), // Espaço para alinhar quando não há botão fechar
        ],
      ),
    );
  }
          ...
          child: Column(
            ...
            children: [
              ...
              AnimatedButton(
                onPressed: _nextPage,
                child: Text(
                  widget.isEditMode != null && widget.isEditMode! ? 'Editar' : 'Começar',
                  style: const TextStyle(
                    color: Colors.white,
                    fontSize: 18,
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...[
                    'Dormir melhor',
                    'Reduzir ansiedade',
                    'Focar nos estudos/trabalho',
                    'Relaxar após um dia agitado',
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...[
                    'Sons da natureza (chuva, mar, vento)',
                    'Batidas suaves (binaural, ASMR)',
                    'Música instrumental relaxante',
                    'Voz suave guiando a meditação',
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...[
                    ...
                    'Durante o dia (pausa mental)',
                    'Antes de estudar ou trabalhar',
                    'Quando acordo',
                    'À tarde ou no pôr do sol',
                  ].asMap().entries.map((entry) {
                    final index = entry.key;
                    final option = entry.value;
                    return AnimatedOptionCard(
          ...
          children: [
            if (_isLoading) ...[
              ...
              AnimatedButton(
                ...
                child: Text(
                  widget.isEditMode != null && widget.isEditMode! ? 'Salvar Alterações' : 'Começar Jornada',
                  style: const TextStyle(
                    color: Colors.white,
                    fontSize: 18,
                    fontWeight: FontWeight.w600,
