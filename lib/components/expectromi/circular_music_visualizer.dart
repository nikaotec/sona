...
class _CircularMusicVisualizerState extends State<CircularMusicVisualizer>
  ...
  CustomPainter _getVisualizerPainter() {
    switch (widget.style) {
      ...
      case VisualizerStyle.sphere:
        return SphereVisualizerPainter(
          rotation: _rotationAnimation.value,
          pulse: _pulseAnimation.value,
          wave: _waveAnimation.value,
          primaryColor: widget.primaryColor,
...
// Painter para visualizador de esfera
class SphereVisualizerPainter extends CustomPainter {
  final double rotation;
  final double pulse;
  final double wave;
  final Color primaryColor;
  ...
  void paint(Canvas canvas, Size size) {
    final center = Offset(size.width / 2, size.height / 2);
    final sphereRadius = size.width * 0.35;
    ...
    // Número de linhas horizontais na esfera
    final horizontalLines = 20;
    
    // Desenhar linhas horizontais da esfera
    for (int i = 0; i < horizontalLines; i++) {
      final t = i / (horizontalLines - 1); // 0 a 1
      final angle = (t - 0.5) * math.pi; // -π/2 a π/2
      
      // Calcular a altura Y da linha
      final y = math.sin(angle) * sphereRadius;
      
      // Calcular o raio da elipse nesta altura
      final ellipseRadius = math.cos(angle) * sphereRadius;
      
      // Adicionar efeito de pulsação baseado na música
      final pulseEffect = 1.0 + (math.sin(wave + i * 0.3) * 0.2 * pulse);
      final animatedRadius = ellipseRadius * pulseEffect;
      
      // Calcular a cor baseada na posição e animação
      final colorT = (math.sin(wave + i * 0.2) + 1) / 2;
      final lineColor = Color.lerp(primaryColor, secondaryColor, colorT)!;
      
      // Calcular a opacidade baseada na distância do centro
     final opacity = (1.0 - angle.abs() / (math.pi / 2)) * 0.8;
      
      paint.color = lineColor.withOpacity(opacity);
      
      // Desenhar a elipse horizontal
      final rect = Rect.fromCenter(
        center: Offset(0, y),
        width: animatedRadius * 2,
        height: animatedRadius * 0.1, // Altura muito pequena para parecer uma linha
      );
    ...
    final glowPaint = Paint()
      ...
      ).createShader(Rect.fromCircle(center: Offset.zero, radius: sphereRadius * 0.8));
    
    canvas.drawCircle(Offset.zero, sphereRadius * 0.8 * pulse, glowPaint);
    
    // Adicionar partículas flutuantes ao redor da esfera
    final particleCount = 12;
    final particlePaint = Paint()..style = PaintingStyle.fill;
    
    for (int i = 0; i < particleCount; i++) {
      final particleAngle = (i / particleCount) * 2 * math.pi + rotation * 2 * math.pi;
      final particleDistance = sphereRadius * (1.2 + math.sin(wave + i) * 0.3);
      
      final particleX = math.cos(particleAngle) * particleDistance;
      final particleY = math.sin(particleAngle) * particleDistance;
      
      final particleSize = 3 + math.sin(wave * 2 + i) * 2;
      final particleOpacity = (math.sin(wave + i * 0.5) + 1) / 2 * 0.7;
      
      particlePaint.color = secondaryColor.withOpacity(particleOpacity);
